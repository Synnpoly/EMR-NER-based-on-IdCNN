...
def augment_with_pretrained(dictionary, ext_emb_path, chars):
    """
    Augment the dictionary with words that have a pre-trained embedding.
    If `words` is None, add every word that has a pre-trained embedding
    to the dictionary, otherwise, only add the words that are given by
    ...
    # We either add every word in the pre-trained file,
    # or only words given in the `words` list to which
    # we can assign a pre-trained embedding
    if chars is None:
