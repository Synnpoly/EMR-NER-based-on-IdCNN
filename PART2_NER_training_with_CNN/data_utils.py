...
def iob2(tags):
    ...
    for i, tag in enumerate(tags):
        ...
        if split[0] == 'B':
            continue
        elif i == 0 or tags[i - 1] == 'O':  # IOB1 to IOB2
            tags[i] = 'B' + tag[1:]
...
def iob_iobes(tags):
    ...
    IOB -> IOBES
    """
    # IOBES格式：一个三字符的DIS类分词，tags为B-DIS，I-DIS，E-DIS；单字符DIS类分词，tag为S-DIS。
    new_tags = []
    for i, tag in enumerate(tags):
        if tag == 'O':
            new_tags.append(tag)
...
def create_input(data):
    ...
    从语料中生成训练所需格式的inputs["chars", 'segs', 'tags']
    """
    inputs = list()
